    ...
    def forward(self, image):
        ...
        for scale in self.scales:
            ...
            if long_size <= crop_size:
                ...
                assert(ph >= height and pw >= width)
                # grid forward and normalize
                h_grids = int(math.ceil(1.0*(ph-self.crop_size_h)/stride)) + 1
                w_grids = int(math.ceil(1.0*(pw-self.crop_size_w)/stride)) + 1
                with torch.cuda.device_of(image):
                    outputs = image.new().resize_(batch,self.nclass,ph,pw).zero_().cuda()
                    count_norm = image.new().resize_(batch,1,ph,pw).zero_().cuda()
                # grid evaluation
                for idh in range(h_grids):
                    for idw in range(w_grids):
                        h0 = idh * stride
                        w0 = idw * stride
                        h1 = min(h0 + self.crop_size_h, ph)
                        w1 = min(w0 + self.crop_size_w, pw)
                        crop_img = crop_image(pad_img, h0, h1, w0, w1)
                        # pad if needed
                        pad_crop_img = pad_image_cityscapes(crop_img, self.module.mean,
                                                 self.module.std, self.crop_size_h, self.crop_size_w)
                        output = module_inference(self.module, pad_crop_img, self.flip)
                        outputs[:,:,h0:h1,w0:w1] += crop_image(output,
                            0, h1-h0, 0, w1-w0)
                        count_norm[:,:,h0:h1,w0:w1] += 1
...
def pad_image_cityscapes(img, mean, std, crop_size_h, crop_size_w):
    b,c,h,w = img.size()
    assert(c==3)
    padh = crop_size_h - h if h < crop_size_h else 0
    padw = crop_size_w - w if w < crop_size_w else 0
    pad_values = -np.array(mean) / np.array(std)
    img_pad = img.new().resize_(b,c,h+padh,w+padw)
    for i in range(c):
        # note that pytorch pad params is in reversed orders
        img_pad[:,i,:,:] = F.pad(img[:,i,:,:], (0, padw, 0, padh), value=pad_values[i])
    assert(img_pad.size(2)>=crop_size_h and img_pad.size(3)>=crop_size_w)
    return img_pad

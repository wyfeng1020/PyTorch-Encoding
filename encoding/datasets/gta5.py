...
class GTA5Segmentation(BaseDataset):
    NUM_CLASS = 19
    #BASE_DIR = 'VOCdevkit/VOC2012'
    BASE_DIR = 'GTA5'
    def __init__(self, root=os.path.expanduser('~/.encoding/data'), split='train', mode=None, transform=None,
                 target_transform=None):
        super(GTA5Segmentation, self).__init__(root, split, mode, transform, target_transform,base_size=512, crop_size=512)
        _gta5_root = os.path.join(self.root, self.BASE_DIR)
        _mask_dir = os.path.join(_gta5_root, 'labels')
        _image_dir = os.path.join(_gta5_root, 'images')
        # train/val/test splits are pre-cut
        #_splits_dir = os.path.join(_cityscapes_root, 'ImageSets/Segmentation')
        if self.mode == 'train':
            _split_f = os.path.join(_gta5_root, 'train.txt')
        elif self.mode == 'val':
            _split_f = os.path.join(_gta5_root, 'val.txt')
        elif self.mode == 'testval':
            _split_f = os.path.join(_gta5_root, 'val.txt')
        elif self.mode == 'test':
            _split_f = os.path.join(_gta5_root, 'test.txt')
        else:
